...
def call_openai(
    ...
    system_prompt = (
        ...
        "Override Rule 3 — Required variable reference implies PASS for presence checks: "
        "if a security-critical attribute is assigned to a required variable "
        "(referenced as var.xxx with NO default in variables.tf), infer PASS for any check "
        "that only verifies the attribute is non-null/non-empty. "
        "A required string variable cannot be null — Terraform enforces this at the module API level. "
        "This applies to attributes like disk_encryption_set_id = var.disk_encryption_set_id "
        "where the variable has no default. "
        "Do NOT apply this rule if the attribute is optional (has a default) or the variable "
